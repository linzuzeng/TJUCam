...
float beta = sqrt(3.0f / 4.0f) * GyroMeasError;   // compute beta   增大后出现噪声，但初始归位和每次转变后偏转回弹速度都较快。
float zeta = sqrt(3.0f / 4.0f) * GyroMeasDrift*30.0f;   // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value   对速度貌似没啥影响。
...
void INITmpu9250(){
  Serial.println("MPU 9250 Setup start!."); 
...
void MPU9250Measure(){
  // If intPin goes high, all data registers have new data
   ...
   if(newData == true) {  // On interrupt, read data
    ...
    //  we'll calculate the accleration value into actual g's
 ...
 //   readGyroData(gyroCount);  // Read the x/y/z adc values
  ...
  Now = micros()+time_bias;
  deltat = ((micros()+time_bias - lastUpdate)/1000000.0f); // set integration time by time elapsed since last filter update
  lastUpdate =Now;
...
void getMres() {
  ...
  // 14 bit resolution (0) and 16 bit resolution (1)
    case MFS_14BITS:
          mRes = 10.*4912./8190.; // Proper scale to return milliGauss//锛侊紒锛�
          break;
    case MFS_16BITS:
...
// Configure MPU6050 gyro and accelerometer for bias calculation
  ...
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity//锛侊紒锛�
...
// At end of sample accumulation, turn off FIFO sensor read
  writeByte(MPU9250_ADDRESS, FIFO_EN, 0x00);        // Disable gyro and accelerometer sensors for FIFO//锛侊紒锛�
  ...
  for (ii = 0; ii < packet_count; ii++) {
    ...
    gyro_bias[2]  /= (int32_t) packet_count;//锛侊紒锛�
...
// Construct the gyro biases for push to the hardware gyro bias registers, which are reset to zero upon device startup
  ...
  data[5] = (-gyro_bias[2]/4)       & 0xFF;//锛侊紒锛�
...
// Push gyro biases to hardware registers
  ...
  writeByte(MPU9250_ADDRESS, ZG_OFFSET_L, data[5]);//锛侊紒锛�
...
// the accelerometer biases calculated above must be divided by 8.
  ...
  accel_bias_reg[2] -= (accel_bias[2]/8);//锛侊紒锛�
...
// Output scaled accelerometer biases for display in the main program
   dest2[0] =0; (float)accel_bias[0]/(float)accelsensitivity; //Changed!!
...
void MPU9250SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
  ...
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, FS<<3); // Set full scale range for the accelerometer to 2 g//锛侊紒锛�
...
// Configure the accelerometer for self-test
   ...
   writeByte(MPU9250_ADDRESS, GYRO_CONFIG,  0xE0); // Enable self test on all three axes and set gyro range to +/- 250 degrees/s//锛侊紒锛�
 ...
 // Configure the gyro and accelerometer for normal operation
   ...
   writeByte(MPU9250_ADDRESS, GYRO_CONFIG,  0x00);  //锛侊紒锛�
   delay(25);  // Delay a while to let the device stabilize
